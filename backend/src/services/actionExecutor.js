...
class ActionExecutor {
  ...
  async execute(action, userId) {
    ...
    if (!validation.valid) {
      console.log(
        `[ActionExecutor] Validation failed for action:`,
        action.type,
        validation.error,
      );
      return {
    ...
    try {
      ...
      console.log(
        `[ActionExecutor] Action ${action.type} executed successfully:`,
        result,
      );
      return {
        success: true,
      ...
      console.error(
        `[ActionExecutor] Action ${action.type} failed:`,
        error.message,
      );
      return {
        success: false,
  ...
  async executeBulk(actions, userId) {
    console.log(
      `[ActionExecutor] Executing ${actions.length} actions for user ${userId}`,
    );
    ...
    for (const action of actions) {
      ...
      if (
        action.type === "sync_calendar_event" &&
        action.data.taskId === "pending"
      ) {
        if (lastCreatedTaskId) {
          action.data.taskId = lastCreatedTaskId;
          console.log(
            `[ActionExecutor] Linked calendar sync to newly created task: ${lastCreatedTaskId}`,
          );
        } else {
          console.log(
            `[ActionExecutor] Warning: sync_calendar_event has pending taskId but no task was created`,
          );
          results.push({
            success: false,
            actionType: action.type,
    ...
    console.log(
      `[ActionExecutor] Bulk execution complete: ${successCount}/${results.length} succeeded`,
    );
  ...
  async createTask(data, userId) {
    ...
    let goal = await Goal.findOne({ title: taskData.category, userId: userId });

    if (!goal) {
      goal = new Goal({
        title: taskData.category,
        description: `Tasks related to ${taskData.category}`,
        progress: 0,
        priority: "medium",
        tasks: [],
        userId: userId,
    ...
    await goal.save();
    await updateGoalProgressForTask(task._id);
  ...
  async updateGoal(data, userId) {
    ...
    if (data.progress !== undefined && updates.progress === undefined)
  ...
  async syncCalendarEvent(data, userId) {
    ...
    if (!user.googleRefreshToken) {
      throw new Error(
        "Google Calendar not connected. Please reconnect with Google to enable calendar sync.",
      );
    }
  ...
  formatResults(executionResult) {
    ...
    if (summary.failed === 0) {
      ...
      const bulkDeletes = results.filter(
        (r) =>
          r.actionType === "delete_all_tasks" ||
          r.actionType === "delete_all_goals",
      );
      if (bulkDeletes.length > 0) {
        const taskDelete = bulkDeletes.find(
          (r) => r.actionType === "delete_all_tasks",
        );
        const goalDelete = bulkDeletes.find(
          (r) => r.actionType === "delete_all_goals",
        );
        const parts = [];
        if (taskDelete)
          parts.push(`${taskDelete.data?.deletedCount || 0} tasks`);
        if (goalDelete)
          parts.push(`${goalDelete.data?.deletedCount || 0} goals`);
